...
function MyBooking() {
  ...
  useEffect(() => {
    fetch(`https://highway-hangout-server.onrender.com/booking/${user.email}`)
      .then((res) => res.json())
  ...
  return (
    <Container
      ...
      className="rounded shadow text-center my-5"
    >
      <h2 className="fw-bold text-center pt-3">
        <span className="text-white fw-normal fst-italic">
          <u>My Booking Info</u>
        </span>
      </h2>
      <p className="fw-light fs-4 mt-3 text-center">
        My Total Booking: {bookingInfo.length}{" "}
      </p>
        ...
        <table class="table">
          ...
          <tbody>
            {bookingInfo.map((book) => (
              <tr key={book._id} className="bg-light">
                <td>{book._id}</td>
                ...
                <td className="text-center">
                  <CloseButton
                    ...
                    className="fs-5 bg-danger rounded-circle ms-2"
